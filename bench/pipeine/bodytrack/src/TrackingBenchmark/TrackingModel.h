...
protected:
    ...
    // standard deviations for each layer of annealed particle filtering
    std::vector<std::vector<float> > mStdDevs; 
    ...
    std::vector<MultiCameraProjectedBody> mProjections; 
    // Image measurement objects for error computation
    std::vector<ImageMeasurements> mImageMeasurements;
...
public:
    ...
    //Load and process new observation data from image files for a given time(frame).  
    //Generates edge maps from the raw image files.
