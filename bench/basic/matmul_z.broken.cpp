...
static int ITER_BASE_CASE;
static int ITER_POWER;
static int REC_BASE_CASE;
static int REC_POWER;
...
// XXX: use ITER_POWER or REC_POWER here?
#define POWER REC_POWER
#define BASE_CASE REC_BASE_CASE
...
#define FUT(k, i, j) &(g_fhandles[fh_index((k), (i), (j), g_nBlocks)])
#define async_matmul(A, B, C, n, i, k, j)				\
  reasync_helper<int,DATA*,DATA*,DATA*,int,int,int,int>			\
  (FUT((k),(i),(j)), matmul_base, (A), (B), (C), (n), (i), (k), (j))
int matmul(DATA *A, DATA *B, DATA *C, int n, int iB, int kB, int jB) { 
  ...
  if (n <= REC_BASE_CASE) {
    async_matmul(A1, B1, C1, n >> 1, iB, kB, jB);
    async_matmul(A1, B2, C2, n >> 1, iB, kB, jB + block_inc);
    async_matmul(A3, B1, C3, n >> 1, iB + block_inc, kB, jB);
    async_matmul(A3, B2, C4, n >> 1, iB + block_inc, kB, jB + block_inc);

    async_matmul(A2, B3, C1, n >> 1, iB, kB + block_inc, jB);
    async_matmul(A2, B4, C2, n >> 1, iB, kB + block_inc, jB + block_inc);
    async_matmul(A4, B3, C3, n >> 1, iB + block_inc, kB + block_inc, jB);
    async_matmul(A4, B4, C4, n >> 1, iB + block_inc, kB + block_inc, jB + block_inc);
  } else {
...
static void do_matmul(DATA *A, DATA *B, DATA *C, int n) {
  ...
  printf("%d x %d with recursive base case %d x %d, iterative %d x %d.\n",
         n, n, REC_BASE_CASE, REC_BASE_CASE, ITER_BASE_CASE, ITER_BASE_CASE);
...
int main(int argc, char *argv[]) {
  ...
  int bSize = 0; // iterative base case
  ...
  if (bSize == 0 || bSize >= rSize) bSize = rSize / 2;

  // Nearest power of 2
  ITER_BASE_CASE = nearpow2(bSize);
  REC_BASE_CASE = nearpow2(rSize);
  if (ITER_BASE_CASE == REC_BASE_CASE) REC_BASE_CASE *= 2;
