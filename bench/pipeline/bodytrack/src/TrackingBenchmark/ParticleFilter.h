...
protected:
  ...
  //resampling bins, new samples, particle likelihoods
  Vectorf mBins, mSamples, mLikelihoods;
  ...
  //random number generators - should be replaced with a single parallel
  //leapfrog generator for better quality random numbers.
  ...
  virtual void CalcWeights(std::vector<Vectorf > &particles);
  //generate new particles distributed by model annealing level std dev
  virtual void GenerateNewParticles(int k);
...
public:
  ...
  //calls model to get observation at given time, and to get likelihoods
  //of each particle
...
ParticleFilter<T>::AddGaussianNoise(Vectorf &p, const Vectorf &stdDevs,
...
template<class T> inline void
ParticleFilter<T>::CalcCDF(const Vectorf &weights, Vectorf &dst)
{
...
//Monte Carlo resampling given a cdf.
//Uses incremental eponential random values to create a sorted uniform
//random sample for fast inverse of the cdf for all samples in 1 pass
template<class T> inline void
ParticleFilter<T>::Resample(Vectorf &cdf, Vectorf &bins, Vectorf &samples, int n)
{
  RandomGenerator r;
...
ParticleFilter<T>::CalcWeights(std::vector<Vectorf > &particles)
{
  bool valid;
  mBestParticle = 0;
...
template<class T> void
ParticleFilter<T>::InitializeParticles(int n)
{
      ...
      for(int i = 0; i < n; i++)
        {
          ...
          this->AddGaussianNoise(p, mModel->StdDevs()[0], mRnd[i]);
        }
      //calculate initial weights and remove any particles that
      //are invalid by model prior
      CalcWeights(mParticles);
      //repeat until minimum number of valid particles is met
      minValid = (int)mParticles.size() >= mMinParticles;
      if(!minValid)
        {
          std::cout << "Warning : initial particle set does not meet "
                    << "minimum number of particles. Resampling.." << std::endl;
        }
    }
...
//by the model annealing parameter
template<class T> void
ParticleFilter<T>::GenerateNewParticles(int k)
{
  int p = 0;
  ...
  for(uint i = 0; i < mBins.size(); i++)
    {
      for(uint j = 0; j < mBins[i]; j++)
        {
          //add new particle for each entry in each bin distributed
          //randomly about duplicated particle
          mNewParticles[p] = mParticles[i];
          this->AddGaussianNoise(mNewParticles[p], mModel->StdDevs()[k], mRnd[p]);
          p++;
        }
    }
}
...
ParticleFilter<T>::Update(fpType timeval)
{
