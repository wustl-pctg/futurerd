...
bool TrackingModel::Initialize(const string &path, int cameras, int layers)
{
    ...
    if(!InitGeometry(path + "BodyShapeParameters.txt")) return false;

    //initialize body pose angles and translations
    if(!LoadInitialState(path + "InitialPose.txt")) return false;

    //initialize pose statistics
    if(!LoadPoseParameters(path + "PoseParameters.txt")) return false;
...
float TrackingModel::LogLikelihood(const vector<float> &v, bool &valid, const int thread)
{
...
inline void GaussianBlur(FlexImage8u &src, FlexImage8u &dst)
{
    float k[] = {0.12149085090552f, 0.14203719483447f, 0.15599734045770f, 
        0.16094922760463f, 0.15599734045770f, 0.14203719483447f, 
        0.12149085090552f};
    FlexImage8u tmp;
...
inline FlexImage8u GradientMagThreshold(const FlexImage8u &src, float threshold)
{
    ...
    {	
        ...
        {	
            ...
            float xg = -0.125f * ph[-1] + 0.125f * ph[1] - 0.250f * p[-1] 
                        + 0.250f * p[1] - 0.125f * pl[-1] + 0.125f * pl[1];
